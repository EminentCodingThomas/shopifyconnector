...
class QueueLineDashboard(models.AbstractModel):
    ...
    def _prepare_query(self, duration, state, table):
        if table == 'shopify_order_data_queue_line_ec':
            queue_type = self._context.get('action_domain')[1][2]
            qry = """SELECT oql.id FROM shopify_order_data_queue_line_ec as oql
            INNER JOIN shopify_order_data_queue_ec as oq ON oq.id=oql.shopify_order_data_queue_id and 
