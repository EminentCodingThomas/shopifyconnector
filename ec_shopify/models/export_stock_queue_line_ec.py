...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    def auto_export_stock_queue_data(self):
        ...
        self.env.cr.execute(
            """update shopify_export_stock_queue_ec set is_process_queue = False where is_process_queue = True""")
        self._cr.commit()

        query = """select distinct queue.id
                                from shopify_export_stock_queue_line_ec as queue_line
                                inner join shopify_export_stock_queue_ec as queue on queue_line.export_stock_queue_id = queue.id
            ...
            for queue_line in self:
                ...
                try:
                    ...
                    if hasattr(error, "response"):
                        ...
                        log_line = common_log_line_obj.create_common_log_line_ec(shopify_instance_id=instance.id,
                                                                                  message=message,
                                                                                  model_name=model,
