...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    def auto_import_order_queue_data(self):
        ...
        self.env.cr.execute(
            """update shopify_order_data_queue_ec set is_process_queue = False where is_process_queue = True""")
        self._cr.commit()

        query = """select queue.id
                from shopify_order_data_queue_line_ec as queue_line
                inner join shopify_order_data_queue_ec as queue on queue_line.shopify_order_data_queue_id = queue.id
