...
class ShopifyCustomerDataQueueLineEpt(models.Model):
    ...
    def sync_shopify_customer_into_odoo(self):
        ...
        query = """select queue.id
            from shopify_customer_data_queue_line_ec as queue_line
            inner join shopify_customer_data_queue_ec as queue on queue_line.synced_customer_queue_id = queue.id
    ...
    def process_customer_queue_lines(self):
        ...
        for queue in queues:
            ...
            if instance.active:
                self.env.cr.execute("""update shopify_product_data_queue_ec set is_process_queue = False where
