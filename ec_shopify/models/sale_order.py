...
class SaleOrder(models.Model):
    ...
    closed_at_ec = fields.Datetime("Closed At", copy=False)
    ...
    def import_shopify_orders(self, order_data_lines, instance):
        ...
        for order_data_line in order_data_lines:
            ...
            if str(instance.import_order_after_date) > date_order:
                message = "Order %s is not imported in Odoo due to configuration mismatch.\n Received order date is " \
                          "%s. \n Please check the order after date in shopify configuration." % (order_number,
                                                                                                  date_order)
                _logger.info(message)
                common_log_line_obj.create_common_log_line_ec(shopify_instance_id=instance.id, message=message,
                                                               model_name='sale.order',
                                                               order_ref=order_response.get("name"),
                                                               shopify_order_data_queue_line_id=order_data_line.id if order_data_line else False)
                continue
            ...
            if instance.is_delivery_multi_warehouse:
                warehouses = sale_order.order_line.filtered(lambda line_item: line_item.warehouse_id_ec).mapped(
                    'warehouse_id_ec')
            ...
            try:
                if sale_order.shopify_order_status == "fulfilled":
                    sale_order.auto_workflow_process_id.shipped_order_workflow_ec(sale_order)
                    ...
                    if sale_order.shopify_order_status == "partial":
                        ...
                        sale_order.process_orders_and_invoices_ec()
    ...
    def set_line_warehouse_based_on_location(self, order, instance):
        ...
        for data in fulfillment_data:
            ...
            order_line.write(
                {'warehouse_id_ec': line_warehouse_id.id if line_warehouse_id else instance.shopify_warehouse_id.id})
        return True
    ...
    def prepare_shopify_order_vals(self, instance, partner, shipping_address,
        ...
        # ordervals = self.create_sales_order_vals_ec(ordervals)
    ...
    def shopify_create_sale_order_line(self, line, product, quantity, product_name, price,
        ...
        # order_line_vals = sale_order_line_obj.create_sale_order_line_ec(line_vals)
        ...
        if instance.apply_tax_in_order == "create_shopify_tax":
            ...
            if line and line.get("tax_lines"):
                if line.get("taxable"):
                    ...
                    # taxable
                    tax_ids = self.shopify_get_tax_id_ec(instance,
                                                          line.get("tax_lines"),
                                                          taxes_included)
                if is_shipping:
    ...
    @api.model
    def shopify_get_tax_id_ec(self, instance, tax_lines, tax_included):
        tax_id = []
        taxes = []
    ...
    def closed_at(self, instance):
        ...
        for sale_order in sales_orders:
            ...
            order.close()
            sale_order.write({'closed_at_ec': datetime.now()})
        return True
    ...
    def search_shopify_location_for_update_order_status(self, sale_order, instance, line_items):
        ...
        if instance.is_delivery_multi_warehouse:
            ...
            shopify_location_id = shopify_location_obj.search(
                [('warehouse_for_order', '=', order_line.warehouse_id_ec.id), ("instance_id", "=", instance.id)])
    ...
    def create_refund_adjustment_line(self, order_adjustments, move_ids):
        ...
        adjustment_product = self.env.ref('ec_shopify.shopify_refund_adjustment_product', False)
    ...
    def action_open_cancel_wizard(self):
        view = self.env.ref('ec_shopify.view_shopify_cancel_order_wizard')
    ...
    def paid_invoice_ec(self, invoices):
        ...
        if self.is_shopify_multi_payment:
            ...
            return True
        super(SaleOrder, self).paid_invoice_ec(invoices)
