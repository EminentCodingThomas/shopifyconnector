...
class ShopifyInstanceEpt(models.Model):
    ...
    def _default_discount_product(self):
        discount_product = self.env.ref('ec_shopify.shopify_discount_product', False)
        return discount_product

    @api.model
    def _default_duties_product(self):
        duties_product = self.env.ref('ec_shopify.shopify_duties_product', False)
        return duties_product

    @api.model
    def _default_shipping_product(self):
        shipping_product = self.env.ref('ec_shopify.shopify_shipping_product', False)
        return shipping_product

    @api.model
    def _default_gift_card_product(self):
        git_card_product = self.env.ref('ec_shopify.shopify_gift_card_product', False)
        return git_card_product

    @api.model
    def _default_custom_service_product(self):
        custom_service_product = self.env.ref('ec_shopify.shopify_custom_service_product', False)
        return custom_service_product

    @api.model
    def _default_custom_storable_product(self):
        custom_storable_product = self.env.ref('ec_shopify.shopify_custom_storable_product', False)
        return custom_storable_product

    @api.model
    def _default_refund_adjustment_product(self):
        refund_adjustment_product = self.env.ref('ec_shopify.shopify_refund_adjustment_product', False)
        return refund_adjustment_product

    @api.model
    def _default_tip_product(self):
        tip_product = self.env.ref('ec_shopify.shopify_tip_product', False)
        return tip_product

    @api.model
    def _default_order_status(self):
        order_status = self.env.ref('ec_shopify.unshipped')
    ...
    def get_total_orders(self):
        ...
        view = self.env.ref('ec_shopify.action_shopify_sales_order').sudo().read()[0]
    ...
    def get_total_products(self):
        ...
        view = self.env.ref('ec_shopify.action_shopify_product_exported_ec').sudo().read()[0]
    ...
    def get_customers(self):
        ...
        view = self.env.ref('ec_shopify.action_shopify_partner_form').sudo().read()[0]
    ...
    def get_refund(self):
        ...
        view = self.env.ref('ec_shopify.action_refund_shopify_invoices').sudo().read()[0]
    ...
    def shopify_action_archive_unarchive(self):
        ...
        if self.active:
            ...
            data_queue_mixin_obj.delete_data_queue_ec(is_delete_queue=True)
        else:
    ...
    def change_auto_cron_status(self):
        try:
            stock_cron_exist = self.env.ref('ec_shopify.ir_cron_shopify_auto_export_inventory_instance_%d' % self.id)
        except Exception as error:
            stock_cron_exist = False
            _logger.info(error)
        try:
            order_cron_exist = self.env.ref('ec_shopify.ir_cron_shopify_auto_import_order_instance_%d' % self.id)
        except Exception as error:
            order_cron_exist = False
            _logger.info(error)
        try:
            order_status_cron_exist = self.env.ref(
                'ec_shopify.ir_cron_shopify_auto_update_order_status_instance_%d' % self.id)
        except Exception as error:
    ...
    def cron_configuration_action(self):
        action = self.env.ref('ec_shopify.action_wizard_shopify_cron_configuration_ec').read()[0]
    ...
    def open_reset_credentials_wizard(self):
        view_id = self.env.ref('ec_shopify.view_reset_credentials_form').id
        action = self.env.ref('ec_shopify.res_config_action_shopify_instance').read()[0]
    ...
    def action_shopify_active_archive_instance(self):
        view = self.env.ref('ec_shopify.view_active_archive_shopify_instance')
        return {
