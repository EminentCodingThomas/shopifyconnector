...
class ResConfigSettings(models.TransientModel):
    ...
    shopify_set_sales_description_in_product = fields.Boolean("Use Sales Description of Odoo Product for shopify",
                                                              config_parameter="ec_shopify.set_sales_description",
    ...
    show_net_profit_report = fields.Boolean(config_parameter="ec_shopify.show_net_profit_report")
    ...
    def execute(self):
        ...
        exist_module = IrModule.search([('name', '=', 'shopify_net_profit_report_ec'), ('state', '=', 'installed')])
        if instance:
    ...
    def download_shopify_net_profit_report_module(self):
        attachment = self.env['ir.attachment'].search(
            [('name', '=', 'shopify_net_profit_report_ec.zip')])
        return {
    ...
    def onchange_is_shopify_digest(self):
        try:
            digest_exist = self.env.ref('ec_shopify.digest_shopify_instance_%d' % self.shopify_instance_id.id)
        except:
            digest_exist = False
        if self.is_shopify_digest:
            ...
            if digest_exist:
                ...
                core_record = shopify_cron.check_core_shopify_cron(
                    "shopify_lib.connector_digest_digest_default")
    ...
    def action_shopify_open_basic_configuration_wizard(self):
        try:
            view_id = self.env.ref('ec_shopify.shopify_basic_configurations_onboarding_wizard_view')
        except:
            return True
    ...
    def action_shopify_open_financial_status_configuration_wizard(self):
        try:
            view_id = self.env.ref('ec_shopify.shopify_financial_status_onboarding_wizard_view')
        except:
            return True
        return self.shopify_res_config_view_action(view_id)
