...
class ShopifyCronConfigurationEpt(models.TransientModel):
    ...
    def update_export_stock_cron_field(self, instance):
        try:
            export_inventory_stock_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_shopify_auto_export_inventory_instance_%d' % instance.id)
        except:
    ...
    def update_import_order_cron_field(self, instance):
        try:
            import_order_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_shopify_auto_import_order_instance_%d' % instance.id)
        except:
    ...
    def import_shipped_order_cron_field(self, instance):
        try:
            import_shipped_order_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_shopify_auto_import_shipped_order_instance_%d' % instance.id)
        except:
    ...
    def import_cancel_order_cron_field(self, instance):
        try:
            import_cancel_order_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_shopify_auto_import_cancel_order_instance_%d' % instance.id)
        except:
    ...
    def update_order_status_cron_field(self, instance):
        try:
            update_order_status_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_shopify_auto_update_order_status_instance_%d' % instance.id)
        except:
    ...
    def update_payout_report_cron_field(self, instance):
        try:
            payout_report_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_auto_import_payout_report_instance_%d' % instance.id)
        except:
            payout_report_cron_exist = False
        try:
            auto_process_bank_statement_cron_exist = instance and self.env.ref(
                'ec_shopify.ir_cron_auto_process_bank_statement_instance_%d' % instance.id)
        except:
        ...
        if instance:
            ...
            if self._context.get('is_calling_from_onboarding_panel', False):
                action = self.env["ir.actions.actions"]._for_xml_id(
                    "ec_shopify.shopify_onboarding_confirmation_wizard_action")
    ...
    def setup_shopify_inventory_export_cron(self, instance):
        try:
            cron_exist = self.env.ref('ec_shopify.ir_cron_shopify_auto_export_inventory_instance_%d' % instance.id)
        except:
            cron_exist = False
        if self.shopify_stock_auto_export:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_shopify_auto_export_inventory")
    ...
    def setup_shopify_import_order_cron(self, instance):
        ...
        if self.shopify_order_auto_import:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_shopify_auto_import_order")
    ...
    def setup_shopify_import_shipped_order_cron(self, instance):
        ...
        if self.shopify_shipped_order_auto_import:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_shopify_auto_import_shipped_order")
    ...
    def setup_shopify_import_cancel_order_cron(self, instance):
        ...
        if self.shopify_cancel_order_auto_import:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_shopify_auto_import_cancel_order")
    ...
    def setup_shopify_update_order_status_cron(self, instance):
        ...
        if self.shopify_order_status_auto_update:
            ...
            if cron_exist:
                ...
                core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_shopify_auto_update_order_status")
    ...
    def setup_shopify_payout_auto_import_payout_report_cron(self, instance):
        ...
        if cron_exist:
            ...
            core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_auto_import_payout_report")
    ...
    def setup_shopify_payout_auto_process_bank_statement_cron(self, instance):
        ...
        """
        cron_exist = self.env.ref('ec_shopify.ir_cron_auto_process_bank_statement_instance_%d' % instance.id, False)
        ...
        if cron_exist:
            ...
            core_cron = self.check_core_shopify_cron("ec_shopify.ir_cron_auto_process_bank_statement")
    ...
    def create_ir_module_data(self, name, new_cron):
        self.env['ir.model.data'].create({'module': 'ec_shopify',
                                          'name': name,
                                          'model': 'ir.cron',
                                          'res_id': new_cron.id,
                                          'noupdate': True})
