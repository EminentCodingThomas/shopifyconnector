...
class SaleOrder(models.Model):
    ...
    def get_fiscal_position_by_warehouse(self):
        ...
        if warehouse and self.partner_id and self.partner_id.allow_search_fiscal_based_on_origin_warehouse:
            ...
            fiscal_position = self.env['account.fiscal.position'].with_context(
                origin_country_ec=origin_country_id, is_amazon_fpos=is_amz_customer,
    ...
    def action_view_stock_move_ec(self):
        """
    ...
    def _prepare_invoice(self):
        ...
        elif self._context.get('journal_ec'):
            invoice_vals.update({'journal_id': self._context.get('journal_ec').id})
        return invoice_vals

    def process_orders_and_invoices_ec(self):
        """
        ...
        for order in self:
            ...
            if not order_lines.filtered(lambda l: l.product_id.type == 'product') and len(
                ...
                continue
            order.validate_and_paid_invoices_ec(work_flow_process_record)
        return True

    def validate_order_ec(self):
        """
    ...
    def validate_and_paid_invoices_ec(self, work_flow_process_record):
        """
        ...
        if work_flow_process_record.create_invoice:
            if work_flow_process_record.invoice_date_is_order_date:
                if self.check_fiscal_year_lock_date_ec():
                    return True
            if work_flow_process_record.sale_journal_id:
                invoices = self.with_context(journal_ec=work_flow_process_record.sale_journal_id)._create_invoices()
            else:
                invoices = self._create_invoices()
            self.validate_invoice_ec(invoices)
            if work_flow_process_record.register_payment:
                self.paid_invoice_ec(invoices)
        return True

    def check_fiscal_year_lock_date_ec(self):
        """
    ...
    def validate_invoice_ec(self, invoices):
        """
    ...
    def paid_invoice_ec(self, invoices):
        """
        ...
        for invoice in invoices:
            if invoice.amount_residual:
                ...
                self.reconcile_payment_ec(payment_id, invoice)
        return True

    def reconcile_payment_ec(self, payment_id, invoice):
    ...
    def auto_shipped_order_ec(self, customers_location, is_mrp_installed=False):
        """
        ...
        for order_line in order_lines:
            ...
            for bom_line in bom_lines:
                self.create_and_done_stock_move_ec(order_line, customers_location, bom_line=bom_line)
            if not bom_lines and order_line.product_id.is_drop_ship_product:
                self.create_and_done_stock_move_ec(order_line, customers_location, vendor_location=vendor_location)
            elif not bom_lines or not is_mrp_installed:
                self.create_and_done_stock_move_ec(order_line, customers_location)
        return True
    ...
    def create_and_done_stock_move_ec(self, order_line, customers_location, bom_line=False, vendor_location=False):
        """
        ...
        if product and product_qty and product_uom:
            vals = self.prepare_val_for_stock_move_ec(product, product_qty, product_uom, vendor_location,
    ...
    def prepare_val_for_stock_move_ec(self, product, product_qty, product_uom, vendor_location, customers_location,
                                       order_line, bom_line):
        ...
        if order_line.warehouse_id_ec:
            location_id = order_line.warehouse_id_ec.lot_stock_id.id
        vals = {
