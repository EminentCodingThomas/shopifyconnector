...
class DataQueueMixinEpt(models.AbstractModel):
    ...
    def delete_data_queue_ec(self, queue_detail=[], is_delete_queue=False):
        ...
        if queue_detail:
            try:
                queue_detail += ['common_log_book_ec']
                queue_detail = list(set(queue_detail))
                for tbl_name in queue_detail:
                    self.delete_data_queue_schedule_activity_ec(tbl_name, is_delete_queue)
                    if is_delete_queue:
    ...
    def delete_data_queue_schedule_activity_ec(self, tbl_name, is_delete_queue=False):
        """
