...
class SaleWorkflowProcess(models.Model):
    ...
    @api.model
    def auto_workflow_process_ec(self, auto_workflow_process_id=False, order_ids=[]):
        """
        ...
        if not order_ids:
            ...
            orders = sale_order_obj.search([('auto_workflow_process_id', 'in', work_flow_process_records.ids),
                                            ('id', 'in', order_ids)])
        orders.process_orders_and_invoices_ec()

        return True

    def shipped_order_workflow_ec(self, orders):
        """
        ...
        mrp_module = product_product_obj.search_installed_module_ec('mrp')
        ...
        for order in shipped_orders:
            order.state = 'sale'
            order.auto_shipped_order_ec(customer_location, mrp_module)

        shipped_orders.validate_and_paid_invoices_ec(self)
        return True
