...
class ReturnPicking(models.TransientModel):
    ...
    sale_order_ec_id = fields.Many2one("sale.order")

    @api.onchange('sale_order_ec_id')
    ...
    def create_returns_ec(self):
        """
        Creates return picking for the direct shipped moves.
        @note: Same as base method but with Customisation.
        """
        for wizard in self:
            new_picking_id, pick_type_id = wizard._create_returns_ec()
        # Override the context to disable all the potential filters that could have been set previously
        ctx = dict(self.env.context)
        ctx.update({
            'default_partner_id': self.sale_order_ec_id.partner_shipping_id.id,
